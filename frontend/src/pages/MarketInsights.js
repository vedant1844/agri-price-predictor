...
export default function MarketInsights() {
  ...
  const fetchData = () => {
    setLoading(true);
    setTimeout(() => {
      const d = MARKET_DATA[filters.commodity] || MARKET_DATA['Cotton'];
  ...
  const chartOpts = {
    ...
    scales: { y: { min: 4000, ticks: { callback: v => '₹' + Number(v).toLocaleString('en-IN'), font: { size: 10 } } } }
  };
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ background: 'white', borderRadius: 14, padding: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: '1rem', marginBottom: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', alignItems: 'end' }}>
            {[
              ...
              { label: 'Commodity', key: 'commodity', opts: ['Cotton','Wheat','Rice','Groundnut','Castor'] },
            ].map(f => (
              <div key={f.key}>
          ...
          {result && (
            <div style={{ animation: 'fadeUp 0.4s ease' }}>
              {/* Warning */}
              <div style={{ background: '#fff8e1', border: '1px solid #ffe082', borderRadius: 10, padding: '10px 16px', fontSize: '0.85rem', color: '#7c6000', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: 8 }}>
                ⚠ Prices not available in {result.state} district. Showing prices from nearby markets.
              </div>
              ...
              <div style={{ background: 'white', borderRadius: 14, padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: '1.5rem' }}>
                ...
                <div style={{ fontSize: '0.8rem', color: '#777', marginTop: '0.8rem', textAlign: 'center' }}>
                  ℹ Government API provides current day prices only. Historical trends not available.
                </div>
              </div>
              ...
              <div style={{ background: 'white', borderRadius: 14, padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ background: '#f1f8f1', borderRadius: 10, padding: '1rem 1.2rem', fontSize: '0.87rem', color: '#4a6b4a', lineHeight: 1.9 }}>
                  ...
                  <strong style={{ color: '#1a2e1a' }}>Variety:</strong> Other &nbsp;|&nbsp;
                  <strong style={{ color: '#1a2e1a' }}>Grade:</strong> Local<br />
                  <strong style={{ color: '#1a2e1a' }}>Commodity:</strong> {result.commodity} &nbsp;|&nbsp;
                  <strong style={{ color: '#1a2e1a' }}>Last Updated:</strong> 11/4/2026
                </div>
              </div>
            </div>
          )}
...
function FetchButton({ onClick }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ padding: '11px 20px', background: hov ? '#2d6235' : '#3a7d44', color: 'white', border: 'none', borderRadius: 8, fontSize: '0.92rem', fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'background 0.2s', width: '100%', marginTop: 0 }}>
      Get Market Data
    </button>
  );
}
